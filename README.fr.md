> 🇬🇧 **English version: [README.md](README.md)** · 🇫🇷 Version française (ce document)
...
Le CLI `h120` se compile directement sous Windows (toolchain MSVC), sans aucune
dépendance en plus de Rust — et ffmpeg dans le `PATH` si l'on veut lire des
formats autres que Y4M :

```powershell
cargo build --release --no-default-features
```

On obtient `target\release\h120.exe`. Le lecteur graphique `h120-play` n'est
**pas compilé sous Windows** : il requiert les bibliothèques GTK4/libadwaita
(par ex. via [gvsbuild](https://github.com/wingtk/gvsbuild)), hors périmètre
ici — pour visionner un flux, décode-le en `.y4m` puis lis-le avec mpv/VLC.
...
Ce projet a entièrement été réalisé par l'intelligence artificielle, grâce à l'outil Claude Code d'Anthropic. 
Les modèles Claude Opus 4.8 et Fable 5 ont été utilisés, Fable 5 ayant servi pour l'implémentation initiale et la
première review de code.

La raison à cela est que je n'ai pas un niveau de compétence en Rust suffisant pour réaliser un tel projet en un temps
raisonnable. Souhaitant davantage réaliser un Proof of Concept plutôt qu'une implémentation complète et fonctionnelle, 
l'usage de l'IA m'a paru pertinente.

Maintenant, concernant la qualification de ce projet comme étant de l'AI Slop, je vous laisse comme seuls juges. 
Le point de vue est compréhensible, d'autant plus que je n'ai pas de compétence particulière en ce qui concerne la
réalisation d'encodeur et de décodeur vidéo. Je suis seulement passionné par ce sujet et ai souhaité recréer ce que je
qualifierais de "Lost Media", l'implémentation originale du codec étant introuvable sur internet.

Vous êtes absolument libre de forker ce projet. Il serait formidable d'avoir un support de ce codec sur FFmpeg et VLC.
Probablement pas à partir du travail effectué sur ce dépôt pour les raisons citées ci-dessus, mais j'espère que cette
initiative ouvrira les portes à une réalisation plus sérieuse à l'avenir.

Je ne demande pas de crédits, tout le travail réalisé est attribuable à l'ITU et à Anthropic.
